...
export const BoatView: React.FC<BoatViewProps> = ({ boat }) => {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-[#006994]">
      {/* Water Layer */}
      <div 
      ...
      >
        {/* Simulated light reflections/waves using a gradient overlay that moves slightly differently */}
         <div 
            ...
            style={{
                ...
                backgroundPosition: `${bgPositionX * 0.5}px ${bgPositionY * 0.5}px`
            }}
         />
      </div>

      {/* HUD */}
      <div className="absolute top-8 right-8 bg-black/60 backdrop-blur-md p-4 rounded-xl border border-white/10 text-white shadow-xl">
      ...
      <div 
        className="absolute top-1/2 left-1/2 w-24 h-48 transform -translate-x-1/2 -translate-y-1/2 transition-transform duration-75 ease-linear"
        style={{ 
      ...
      >
         ...
         {Math.abs(boat.speed) > 0.5 && (
            <div className="absolute top-[90%] left-1/2 transform -translate-x-1/2 w-full h-32 overflow-hidden opacity-60">
                 <div className="w-0 h-0 border-l-[30px] border-r-[30px] border-t-[100px] border-l-transparent border-r-transparent border-t-white/40 blur-sm"></div>
            </div>
         )}
         ...
         <div className="w-full h-full relative drop-shadow-2xl">
             {/* Main Body */}
             <div className="absolute inset-0 bg-gray-200 rounded-[50%_50%_5px_5px] clip-path-polygon">
                  <div className="absolute top-0 left-0 right-0 h-16 bg-gray-300 rounded-[50%_50%_0_0] border-b border-gray-400"></div>
             </div>
             {/* Deck details */}
             <div className="absolute top-10 left-2 right-2 bottom-2 bg-[#b08968] rounded-t-[40px] border border-black/10">
                 <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 w-16 h-12 bg-white rounded shadow-lg border border-gray-300">
                     {/* Seat */}
                     <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-10 h-8 bg-gray-800 rounded"></div>
                 </div>
                 {/* Motor */}
